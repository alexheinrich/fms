...
if %DEMO_ARG% == "demo" (
  echo Making fms Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making fms Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild fms-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild fms.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild fms.sln /t:fms-app;fms-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\fms.aaxplugin\Contents\Win32\fms.aaxplugin --out .\build-win\aax\bin\fms.aaxplugin\Contents\Win32\fms.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\fms.aaxplugin\Contents\x64\fms.aaxplugin --out .\build-win\aax\bin\fms.aaxplugin\Contents\x64\fms.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\fms.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\fms.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "fms Installer" ".\installer\fms Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\fms Installer.exe" ".\installer\fms Demo Installer.exe"
  REM -del ".\installer\fms Installer.exe"
  REM -)
