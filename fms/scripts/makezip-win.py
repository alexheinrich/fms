...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\fms Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\fms Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\fms manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\fms.vst3\Contents\\x86_64-win\fms.vst3",
      projectpath + "\\build-win\fms_x64.exe"  
    ]

  zipname = "fms-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\fms-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\fms-app_x64.pdb"  
  ]
