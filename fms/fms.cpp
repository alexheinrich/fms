...
fms::fms(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "fms", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void fms::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void fms::ProcessMidiMsg(const IMidiMsg& msg)
{
...
void fms::OnParamChange(int paramIdx)
{
