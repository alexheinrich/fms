/* Declares the fms Audio Worklet Node */

class fmsController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "fms", options);
  }

  static importScripts (actx) {
    var origin = "https://alexheinrich.github.io/fms/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/fms-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/fms-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
